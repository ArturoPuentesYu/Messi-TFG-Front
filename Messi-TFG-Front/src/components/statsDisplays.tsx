...
export const StatPage = () => {
      ...
      .then((response) => {
        if (response.status === 'ok') {
          ...
          const result = data.reduce(
            (
              totals: {
                partidos_jugados: any
                goles: any
                asistencias: any
                tarjetas_amarillas: any
                tarjetas_rojas: any
                num_titulos: any
              },
              player: {
              ...
              tarjetas_amarillas:
                totals.tarjetas_amarillas + player.tarjetas_amarillas,
          ...
          const calc = {
            ...
            asists_partidos: (result.asistencias / result.partidos_jugados).toFixed(
              2
            ),
            gol_asist_partidos: (
              (result.goles + result.asistencias) /
              result.partidos_jugados
            ).toFixed(2)
          }
          setTotalMessi(calc)
          setObjStats(calc)
  ...
  const setObject = (index: number) => {
    ...
    const calc = {
      ...
      goles_partidos: (
        selectedPlayer.goles / selectedPlayer.partidos_jugados
      ).toFixed(2),
      asists_partidos: (
        selectedPlayer.asistencias / selectedPlayer.partidos_jugados
      ).toFixed(2),
      gol_asist_partidos: (
        (selectedPlayer.goles + selectedPlayer.asistencias) /
        selectedPlayer.partidos_jugados
      ).toFixed(2)
    }
    setObjStats(calc)
  }
              ...
              <div className="mx-[2%] my-8 sm:my-12">
                ...
                <dl className="grid grid-cols-2 gap-4 lg:grid-cols-3 2xl:grid-cols-6">
                  <div className="flex flex-col rounded-lg border border-gray-100 bg-slate-50 px-4 py-8 text-center">
                    ...
                    <dd className="text-2xl font-extrabold text-stone-800 md:text-5xl">
                      <AnimatedNumber
                        finalNumber={club.partidos_jugados}
                        duration={2000}
                      />
                    </dd>
                  </div>
                  ...
                  <div className="flex flex-col rounded-lg border border-gray-100 bg-slate-50 px-4 py-8 text-center">
                    ...
                    <dd className="text-2xl font-extrabold text-stone-800 md:text-5xl">
                      <AnimatedNumber finalNumber={club.goles} duration={2000} />
                    </dd>
                  </div>
                  ...
                  <div className="flex flex-col rounded-lg border border-gray-100 bg-slate-50 px-4 py-8 text-center">
                    ...
                    <dd className="text-2xl font-extrabold text-stone-800 md:text-5xl">
                      <AnimatedNumber
                        finalNumber={club.tarjetas_amarillas}
                        duration={2000}
                      />
                    </dd>
                  </div>
                  <div className="flex flex-col rounded-lg border border-gray-100 bg-slate-50 px-4 py-8 text-center">
                    ...
                    <dd className="text-2xl font-extrabold text-stone-800 md:text-5xl">
                      <AnimatedNumber
                        finalNumber={club.tarjetas_rojas}
                        duration={2000}
                      />
                    </dd>
                  </div>
                  <div className="flex flex-col rounded-lg border border-gray-100 bg-slate-50 px-4 py-8 text-center">
                    ...
                    <dd className="text-2xl font-extrabold text-stone-800 md:text-5xl">
                      {(
                        (club.goles + club.asistencias) /
                        club.partidos_jugados
                      ).toFixed(2)}
                    </dd>
                  </div>
                </dl>
